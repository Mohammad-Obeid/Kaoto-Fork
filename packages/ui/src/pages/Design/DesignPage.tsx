...
export const DesignPage: FunctionComponent<{ fallback?: ReactNode; additionalToolbarControls?: JSX.Element[] }> = (
  ...
  const [modalVariant, setModalVariant] = useState<AlertVariant>(AlertVariant.success);
  ...
  const confirmUpload = async () => {
    ...
    try {
      ...
      if (response.ok) {
        showModal('Upload success', text, AlertVariant.success);
      } else {
        showModal('Upload failed', text, AlertVariant.danger);
      }
    } catch (error) {
      showModal('Upload error', `${error}`, AlertVariant.danger);
    }
  };
  ...
  //     if (response.ok) {
  //       showModal('Test success', JSON.stringify(data, null, 2), AlertVariant.success);
  //     } else {
  //       showModal('Test failed', JSON.stringify(data, null, 2), AlertVariant.danger);
  //     }
  //   } catch (error) {
  //     showModal('Test error', `${error}`, AlertVariant.danger);
  //   }
  // };

  // const handleValidate = async () => {
  //   try {
  //     const requestBody = { yamlContent };
  ...
  //     const data = await response.json();
  //     if (response.ok) {
  //       showModal('Validation success', 'YAML is valid.', AlertVariant.success);
  //     } else {
  //       showModal('Validation failed', JSON.stringify(data, null, 2), AlertVariant.danger);
  //     }
  //   } catch (error) {
  //     showModal('Validation error', `${error}`, AlertVariant.danger);
  //   }
  // };

  const myButtons: JSX.Element[] = [
      ...
      <ActionConfirmationModalContextProvider>
        ...
        <Modal
          ...
          style={{
            ...
            backgroundColor:
              modalVariant === AlertVariant.success
                ? 'rgba(0, 128, 0, 0.95)' // أخضر شفاف للسكسيس
                : 'rgba(255, 0, 0, 0.95)', // أحمر شفاف للإيرور
        ...
        >
          <div
            style={{
              ...
              fontSize: 16,
              lineHeight: 1.5,
              color: '#ffffff', // خط أبيض في كل الحالات
              fontWeight: 600,
              userSelect: 'text',
              minWidth: 320,
              maxWidth: 420,
